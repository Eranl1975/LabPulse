...
export default function UserMenu({ profile, userEmail }: { profile: Profile | null; userEmail?: string | null }) {
  ...
  // Fall back to userEmail if profile row is missing (authenticated but no DB row)
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        {profile && (<span style={{ fontSize: '0.625rem', color: '#14b8a6', fontWeight: 600, background: 'rgba(20,184,166,0.15)', padding: '0.125rem 0.375rem', borderRadius: '4px' }}>
        ...
        <>
          ...
          <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 0.5rem)', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '10px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', minWidth: '200px', zIndex: 50, overflow: 'hidden' }}>
            <div style={{ padding: '0.75rem 1rem', borderBottom: '1px solid #f1f5f9' }}>
              ...
              <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{displayEmail}</div>
            </div>
            {profile?.role === 'admin' && (
